...
The anchor image: **weather, not narrative.** A good focus soundscape is a
room you work inside. It has a floor (the drone), walls that slowly change
color (pads), and occasionally a bird goes past the window (the motif). You
notice it when you arrive and when you leave; in between it holds you without
asking anything.
...
- **Eno, *Music for Airports* (1978)** — the architecture. "2/1" is literally
  tape loops of unequal length: seven sung notes, each looping on its own
  cycle (~17s to ~31s), so their alignment never repeats within a listening
  session. Our stems are the same trick with better tooling: loop lengths in
  bars chosen pairwise non-multiple, so the combined pattern only recurs at
  the least common multiple — hours.
