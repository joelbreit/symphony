"""Composition framework for "The Box Is Full" (after Korobeiniki).

Adapted from the symphony repo's compose/common.py: same note DSL and
velocity-dynamics writer, with a roster swap (square-wave lead in place of
celesta), a full drum-kit percussion map, and a tempo-map integrator so
quarterLength offsets convert to exact seconds for the web manifest.

Offsets are music21 quarterLengths throughout. Real time comes from
...
RNG = random.Random(1989)  # deterministic builds (Game Boy Tetris, 1989)
...
PERC_KEYS = {
    ...
    'china': 52, 'ridebell': 53, 'tamb': 54, 'splash': 55, 'cowbell': 56,
    'crash2': 57, 'shaker': 70, 'claves': 75, 'wbh': 76, 'wbl': 77,
    'trimute': 80, 'tri': 81,
}
...
class Orchestra:
    def __init__(self):
        ...
        self.tempos: list[tuple[float, float]] = []   # (offset_ql, bpm)
        self.marks: list[tuple[str, float]] = []      # (label, offset_ql) sections
        self.cues: dict[str, float] = {}              # named offsets for moments
    ...
    def tempo(self, offset: float, bpm: float, text: str | None = None):
        ...
        self.tempos.append((float(offset), float(bpm)))
    ...
    def mark(self, label: str, offset: float):
        """Record a section boundary for the web manifest."""
        self.marks.append((label, float(offset)))

    def cue(self, name: str, offset: float):
        """Record a named instant (for manifest moments / emblem trigger)."""
        self.cues[name] = float(offset)

    # -- timing -----------------------------------------------------------
    def seconds(self, offset: float) -> float:
        """Convert a quarterLength offset to seconds via the tempo map."""
        ts = sorted(self.tempos)
        if not ts:
            return offset * 0.5
        sec = 0.0
        for i, (off, bpm) in enumerate(ts):
            nxt = ts[i + 1][0] if i + 1 < len(ts) else None
            if offset <= off:
                break
            span_end = offset if (nxt is None or offset < nxt) else nxt
            sec += (span_end - off) * 60.0 / bpm
            if nxt is None or offset < nxt:
                break
        # time before the first tempo mark (shouldn't happen) at first bpm
        first_off, first_bpm = ts[0]
        if offset < first_off:
            return offset * 60.0 / first_bpm
        return sec + min(offset, first_off) * 60.0 / first_bpm
    ...
    def perc(self, offset: float, notes, vel='mf'):
        """notes: 'kick:q r:e sn:e' style string or [(key|midi, ql), ...]."""
        events = []
