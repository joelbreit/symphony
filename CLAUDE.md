...
`web/public/pieces/<id>/` (manifest + notes JSON + score JSON + audio).
`PIECES.md` is the authoritative packaging spec; `tools/midi_to_piece.py`
converts arbitrary MIDI into a package. The Window's package is generated by
its own `export_web.py` instead (it reads the music21 sources directly). Both
call `tools/midi_to_score.py`, which turns the same MIDI into quantized
notation data for the player's sheet-music mode (roll/score toggle: engraved
staves via VexFlow, live cursor synced through the tempo map, click-to-seek);
it can also be run alone to refresh `score/*.json` after manifest key edits.
